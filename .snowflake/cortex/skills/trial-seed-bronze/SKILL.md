...
description: Stand up the Employee 360 medallion on a trial account with NO external access — the offline seeder path. Instead of extracting the live mock API over a tunnel, load the same synthetic OmniHR + Harvest JSON straight into the Bronze VARIANT tables with src/seeders/seed_bronze.sh (driven by profiles_omnihr.json / profiles_harvest.json), then run the SAME committed Silver→Gold→semantic→document SQL as the live path. Deterministic (SEED=42) so it byte-matches the API path. Use when an attendee on a trial account wants to follow along without a tunnel or External Access Integration.
tools:
- read_file
- run_shell_command
- ask_user_question
...
- The **offline seeder path**: a Snowflake **trial account**, which cannot create an External Access
  Integration, so it can't pull the mock API over a tunnel.
...
- Keywords: trial account, no External Access, no EAI, seeder, seed Bronze, offline, follow along.

This is the offline twin of the live ingest: `seed_bronze.sh` is the no-tunnel replacement for
`medallion-build`'s Bronze *ingest* (the only EAI-bound step). Everything **downstream is identical** — the
...
the extraction subagent + `medallion-build`'s Silver + Gold review hooks against the seeded Bronze — see
step 3.)

# Prerequisites

1. **A trial-account connection** in `~/.snowflake/connections.toml` (the script has no default — always pass
   `--connection <your-connection>`). The kit never stores credentials.
...
snow sql -c <your-connection> --role ACCOUNTADMIN -f ../05_semantic_analyst.sql      # GOLD.HR_ANALYST (Cortex Analyst)
snow sql -c <your-connection> --role ACCOUNTADMIN -f ../01_document_ingestion.sql    # COMPANY_KB_SEARCH (Cortex Search)
```
...
already seeded, so the only EAI-bound step (Bronze ingest) is done. First run **`api-schema-extraction` in
offline mode** — its dedicated subagent derives `build/extraction_map.json` from the seeded Bronze VARIANT (or
`schema_spec.json`), no tunnel needed. Then hand the map + seeded Bronze to **`medallion-build`** and generate
**Silver then Gold** with its per-layer review hook (an `ask_user_question` selection popup — **Run it** ·
**Revise** · **Show full SQL** · **Skip**, plus the auto free-form; skip its Bronze/EAI layer). Keep the curated
Gold rollups + `05`/`01` pinned to the committed reference so the dashboard's hardcoded names still resolve.
See `medallion-build` → **Trial / offline mode**.
...
- **Always pass `--connection <your-connection>`** — the seeder has no default connection.
...
`cd src/seeders && ./seed_bronze.sh --connection <your-connection>`, `CALL SP_BUILD_SILVER()`, runs `04`/`05`/`01`,
...
CoCo edits the `CANDIDATES` row count in `profiles_omnihr.json`, re-runs `seed_bronze.sh --connection <your-connection>`
