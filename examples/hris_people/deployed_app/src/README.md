...
| 3·B | `seeders/seed_bronze.sh` → `CALL SILVER.SP_BUILD_SILVER()` | **Offline Bronze load (trial / no EAI).** Generates the same JSON locally and loads it into Bronze VARIANT — no tunnel — then the same flatten. The full medallion without External Access. *(Quick shortcut: `seeders/seed_*.sh --schema SILVER` writes Silver directly, skipping Bronze.)* |
