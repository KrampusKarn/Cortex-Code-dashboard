#!/usr/bin/env bash
# Orchestrator: reseed ALL three sources for DASHBOARD_SPS in FK-safe order
# (OmniHR -> Harvest -> Lattice). Flags pass through to each per-source seeder.
...
if [ "$RESET" = 1 ] && [ "$DRY" = 0 ]; then
  echo "⚠️  Full reseed: this TRUNCATEs and reloads ALL 35 data tables (OmniHR + Harvest + Lattice)"
...
echo ""; echo "=== 1/3 OmniHR (HR / org / recruitment / leave) ==="
bash "$HERE/seed_omnihr.sh" "$@"
echo ""; echo "=== 2/3 Harvest (delivery / time / billing) ==="
bash "$HERE/seed_harvest.sh" "$@"
echo ""; echo "=== 3/3 Lattice (performance) ==="
bash "$HERE/seed_lattice.sh" "$@"
